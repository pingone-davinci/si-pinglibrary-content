 ...
 */
  ...
  */
  ...
  static addFormValidation({
    ...
    // Add required field indicators if specified
    ...
    // Disable the default browser validation
    ...
    if (makePasswordToggle) {
      ...
      passwordFields.forEach((field) => {
        ...
        DaVinciFormUtils.makePasswordToggle(containerId);
      });
    }
    ...
    const setValidationMessage = (element) => {
      ...
      element.setAttribute("data-valid", !message); // Write validation result to an attribute

      updateFeedbackElement(element, message);

      if (invalidFieldBorderColor) {
        element.style.borderColor = message ? invalidFieldBorderColor : "";
      }
    };

    const updateFeedbackElement = (element, message) => {
      const parent = element.parentNode;
      let feedbackElement = parent.querySelector("div.custom-invalid-feedback");
    ...
    const validateOnChange = (element) => {
      element.addEventListener("input", () => {
        if (element.dataset.wasInvalid === "true" || element.dataset.firstBlur === "true") {
          setValidationMessage(element);
          element.classList.toggle("custom-invalid", !element.checkValidity());
        }
      });
    };
    ...
    const validateOnBlur = (element) => {
      element.addEventListener("blur", () => {
        setValidationMessage(element);
        element.classList.toggle("custom-invalid", !element.checkValidity());
        element.dataset.firstBlur = "true";
        element.removeAttribute("data-was-invalid");
      });
    };

    const validateOnFocus = (element) => {
      element.addEventListener("focus", () => {
        if (element.dataset.firstBlur === "true" && !element.checkValidity()) {
    ...
    const validateForm = (event) => {
      ...
      Array.from(form.elements).forEach((element) => {
        if (element.willValidate) {
          ...
          validateOnFocus(element);  // Ensure focus validation is applied
        }
      });
    ...
    // Initial application of change, blur, and focus validation to required fields
    Array.from(form.elements).forEach((element) => {
      if (element.willValidate) {
        validateOnChange(element);
        validateOnBlur(element);
        validateOnFocus(element);
      }
    });
      ...
      DaVinciFormUtils.activatePasswordValidation({
        ...
        onPasswordValid: (isValid) => {
          ...
          if (!isValid) {
            ...
            passwordField.classList.remove("custom-invalid");
            updateFeedbackElement(passwordField, "");
          }
        },
        onPasswordsMatch: (doMatch) => {
