...
class DaVinciFormUtils {
   ...
   * @param {boolean} [options.disableSubmitButtonUntilValid=false] - Whether to disable the submit button until valid. (Optional).
   * @returns {void}
   */
  static addDaVinciFormValidation({
    ...
    submitButton = document.querySelector('button[data-skbuttontype="form-submit"][type="submit"]');
    ...
    // Update Password Form Types require password and confirm password fields
    ...
    if (formType === FORM_TYPE_UPDATE_PASSWORD) {
      ...
      // Update password forms require a password policy
      if (passwordPolicy) {
    ...
    // Add required field indicators if option set
    if (addRequiredFieldIndicators) {
      DaVinciFormUtils.addRequiredFieldIndicators(formId);
    }

    // Set focus on the first field if option set
    ...
    // Set password toggle if option set
    ...
    // Password popup policy normalization
    ...
    // Update password policy popup based on password input
    ...
    // Create password policy popup for update password forms
    if (formType === FORM_TYPE_UPDATE_PASSWORD) {
    ...
    // Set validation message for form fields based on attributes and validity
    ...
    const validateOnChangeAndBlur = (element) => {
      ...
      element.addEventListener("input", () => {
        ...
        if (disableSubmitButtonUntilValid && submitButton) {
          submitButton.disabled = !form.checkValidity();
        }
      });

      // Validate required fields when field loses focus
      ...
      // Add listener to display password policy popup when field receives focus
    ...
    submitButton.addEventListener("click", function (event) {
      ...
      if (allValid) {
        // Continue with the form submit (click) event
        console.log("Is all valid: true...attempting to continue click")
        event.target.click();
